...
- Code: `src/monetization/ads.native.tsx` — Google `TestIds` when `__DEV__` or `EXPO_PUBLIC_ADMOB_USE_TEST_IDS=1`. **iOS live ads will not fill until the app is listed on the App Store** (Google policy) — keep `USE_TEST_IDS=1` + `ALLOW_FORCE_FREE=1` on TF builds. While `USE_TEST_IDS=1`, banners ignore Pro (only rewarded mute suppresses) so TF can verify AdMob; live App Store binary must set `0`. Prefer in-screen Home banner dock over NativeTabs.BottomAccessory.; flip both off/0 for the App Store binary. Pro suppresses ads; Settings → Force free works when `ALLOW_FORCE_FREE=1`.
