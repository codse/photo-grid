...
- Code: `src/monetization/ads.native.tsx` — Google `TestIds` when `__DEV__` or `EXPO_PUBLIC_ADMOB_USE_TEST_IDS=1`. **EAS `production` sets `USE_TEST_IDS=0`** (live units) for App Store / release binaries. Pre-store TF debugging can temporarily set `=1` (+ optional `ALLOW_FORCE_FREE=1`); while TestIds are on, banners ignore Pro so AdMob plumbing can be verified. Note: Google often won’t fill *live* iOS inventory until the app is listed on the App Store — empty banners pre-listing are expected. Prefer in-screen Home banner dock over NativeTabs.BottomAccessory. Pro suppresses ads on live builds; Settings → Force free only when `ALLOW_FORCE_FREE=1`.
